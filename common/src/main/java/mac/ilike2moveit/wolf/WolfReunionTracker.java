...
public final class WolfReunionTracker {
    ...
    public static void clientTick(Minecraft minecraft) {
        ...
        if (level == null || minecraft.player == null || minecraft.getConnection() == null) {
            if (connectionIdentity != null) {
                ...
                MoveItCore.LOGGER.info("[Wolf Reunion] session closed; per-UUID states cleared.");
            }
            return;
        }
        ...
        if (connectionIdentity != currentConnection) {
            ...
            MoveItCore.LOGGER.info(
                    "[Wolf Reunion] new client session; greeting fully armed " +
                            "(absence >100 blocks -> return <=8 blocks, near-zone exit >=11, no minimum wait).");
        }
    ...
    public static int queueDiagnosticReturn() {
        ...
        if (nearest == null) {
            minecraft.player.displayClientMessage(Component.literal(
                    "[Wolf Reunion] No tamed, sitting dog within 8 blocks."), false);
            return 0;
        }
        ...
        if (result.event() != WolfReunionState.Event.RETURN_QUEUED) {
            minecraft.player.displayClientMessage(Component.literal(
                    "[Wolf Reunion] That dog already has a pending sequence."), false);
            return 0;
        }
        ...
        MoveItCore.LOGGER.info(
                ...
                result.sequence());
        minecraft.player.displayClientMessage(Component.literal(
                "[Wolf Reunion] Diagnostic queued for " + nearest.getUUID() +
    ...
    private static void traceEmf(UUID uuid, long now, Map<String, Float> variables) {
        ...
        if (now > trace.untilTick) {
            MoveItCore.LOGGER.info("[Wolf Reunion Trace] end uuid={} sequence={} reason=timeout",
                    uuid, trace.sequence);
            TRACES.remove(uuid);
            return;
        }
        ...
        if (!snapshot.equals(trace.lastSnapshot)) {
            ...
            MoveItCore.LOGGER.info("[Wolf Reunion Trace] uuid={} sequence={} tick={} {}",
