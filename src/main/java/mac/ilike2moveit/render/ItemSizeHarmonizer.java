 ...
 * {@code rightRotation.y/z}, just like vanilla). The model box's <b>eight corners</b> are transformed
...
public final class ItemSizeHarmonizer {
    ...
    private static void applyItemTransform(Vector3f point, ItemTransform transform, boolean leftHand) {
        ...
        degreesToQuaternion(transform.rightRotation, mirror).transform(point);
    ...
    private static float[] narrowToOpaquePixels(BakedModel model, float[] box) {
        ...
        if (maxX < 0) {
            return box;   // textura entera transparente: nos quedamos con la caja cruda
        }
